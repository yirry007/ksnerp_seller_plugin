...
export const Tmall = {
    ...
    /** 验证需不需要采购当前页面的商品 */
    hasMatched: (cb) => {
        const supply_code = Tmall._getSupplyCode();
    ...
    /** 自动拾取天猫订单列表的订单，订单商品，发货以及快递信息 */
    collectOrder: async (cb) => {
        /** 先清空数据容器 */
        Tmall._itemInfoWithDelivery = [];
        ...
        const result = await Tmall._getItemInfoWithDelivery(orderItems);
    ...
    _getSupplyCode: () => {
        ...
        /** 商品详细页的url验证 */
        if (currentUrl.indexOf('detail.tmall.com') === -1) {
            return false;
        } else {
    ...
    _getItemInfoWithDelivery: async (itemTrList) => {
        ...
        Tmall._itemInfoWithDelivery.push(_item);
        await Tmall._getItemInfoWithDelivery(itemTrList);

        return Tmall._itemInfoWithDelivery;
    },
};